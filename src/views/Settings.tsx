...
export default function Settings() {
  ...
  async function handleSaveSuite() {
    ...
    if (!trimmed) {
      setError("El nombre de la suite no puede estar vacío.");
      return;
    }
    setBusy(true);
    try {
  ...
  async function handleReindex() {
    ...
    try {
      ...
      setReindexMessage(`index.json regenerado en ${path}`);
    } catch (e) {
      setError(String(e));
    } finally {
      setBusy(false);
  ...
  return (
    ...
    >
      ...
      {reindexMessage && <Alert tone="success" title="Listo">{reindexMessage}</Alert>}
      ...
      <section>
        ...
        >
          Se actualiza automáticamente cuando creas casos con prefijos nuevos.
        </p>
      </section>
      ...
      <Modal
        ...
        title={editor?.originalName ? "Editar suite" : "Nueva suite"}
        footer={
          <>
            <Button variant="ghost" onClick={() => setEditor(null)}>Cancelar</Button>
            <Button variant="primary" onClick={handleSaveSuite} disabled={busy}>Guardar</Button>
          </>
        }
      >
        {editor && (
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--adc-space-3)" }}>
            <div className="adc-field">
              <label className="adc-label">Nombre</label>
              <input
                className="adc-input"
                value={editor.name}
                onChange={(e) =>
            ...
            <div className="adc-field">
              <label className="adc-label">
                Casos ({editor.caseIds.size} seleccionado{editor.caseIds.size === 1 ? "" : "s"})
              </label>
              <SuiteCaseSelector
                cases={cases}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="danger" onClick={handleDeleteSuite} disabled={busy}>Borrar</Button>
          </>
        }
      >
        <p>
          Vas a borrar la suite <code>{confirmDelete}</code>. Los casos individuales no se tocan.
        </p>
      </Modal>
    </main>
  );
